...
function ungallery() {	
	...
	while ($filename = readdir($dp)) {
		if (!is_dir($pic_root.$win_gallery_path. "\\". $filename))  {  // If it's a file, begin
				...
				if (in_array(substr($filename, -3), $pic_types)) $pic_array[] = $filename;							// If it's a picture, add it to thumb array
					...
					if (in_array(substr($filename, -3), $movie_types)) $movie_array[$filename] = size_readable(filesize($pic_root.$win_gallery_path. "/". $filename)); 	
